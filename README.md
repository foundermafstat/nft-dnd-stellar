...
DND ARENA ZK (Dice Roll & Loot): CB4DFJLB34UCIEAOBIKLLEJ6DMIUBBNUKYBKBZIDJILUFR2AQCNVWN4Z
DND Token (Item NFTs): CB2RIG4PPLQNW4EBL6WRWU4VSHO4GCH722XT4BLXMJ7NJM34ZI2OC2RU
Fate Verifier (ZK Primitives): CCIGFXIXSFUCYWRJVCXFOBAC2KRS4C4AQZRFGSM6A7H3KRXV552CGMCW
Relic Registry (Item Templates): CAOXCTX57AJJA6D2QN5UXDF3OHDZBDBS2J6SKSHI7TOFTMKGKD5Y5H2X
Adventure Vault (Session Storage): CCIIZ2MFPGV3SIRM3K2ZJFVPG6LMDCROTYUDTKI2GB6OHWZLRSSTGQ6J
Game Hub (Session Director): CB4VZAT2U3UC6XFK3N23SKRF2NDCMP3QHJYMCHHFMZO7MRQO6DQ2EMYG
...
│   ├── soroban/dnd_arena_game/ # ZK Dice & Loot Resolution contract (Rust)
│   ├── soroban/dnd_token/      # NFT item contract (Rust)
│   ├── soroban/relic_registry/ # Item Registry contract (Rust)
│   └── soroban/fate_verifier/  # ZK Verifier contract (Rust)
│
...
### 11. DnD Arena Game (ZK & Loot)

The `DndArenaGameContract` (`contracts/soroban/dnd_arena_game/src/lib.rs`) is the high-stakes execution layer of the game. It integrates Zero-Knowledge verification with automated loot distribution and peer-to-peer wagering.

**Key Features:**

- **Game Hub Integration**: Naturally interacts with the `GameHub` contract to handle session starts, wagers (points), and settlements.
- **ZK Dice Roll Verification**:
  - Implements `verify_dice_proof` using **RISC Zero zkVM**.
  - Players must submit a cryptographic `seal` and `journal` containing their pre-calculated dice roll.
  - The contract verifies the proof against the official `IMAGE_ID`, ensuring the roll was generated by a fair, untampered guest program before the player can commit their turn.
- **Deterministic Loot Generation**:
  - Upon `resolve_game`, the contract uses an on-chain PRNG seeded by the verified dice values and the session ID.
  - Generates randomized **Gold** and **Item Drops** (Item IDs mapping to the Relic Registry) directly in the contract state.
  - Prevents "lucky guess" exploitation by anchoring randomness in the verified ZK inputs.
- **Winner Determination**: Automated settlement. If Player 1's verified roll ≥ Player 2's, Player 1 is declared the winner and the outcome is pushed back to the Game Hub for prize distribution.

**Workflow:**
`start_game` (Locks wagers) → `verify_dice_proof` (ZK validation) → `commit` (Action lock) → `resolve_game` (Winner + Loot Generation) → `end_game` (Hub closure).
