...
export default function FreighterAuthButton({ onAuthenticated }: AuthButtonProps) {
    ...
    return (
        <div className="flex items-center gap-4">
            {error && <span className="text-red-400 text-xs font-serif max-w-xs">{error}</span>}
            <Button
                onClick={handleConnect}
                disabled={loading}
                className="bg-amber-700 hover:bg-amber-600 text-stone-100 font-serif shadow-[0_0_15px_rgba(217,119,6,0.5)] hover:shadow-[0_0_25px_rgba(217,119,6,0.7)] transition-all flex items-center gap-2 border border-amber-500/50"
            >
